 ...
 */
export function createHookServer(opts) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (method === "POST" && url === "/v1/metrics") {
                ...
                if (verbose)
                    log(`otel metrics: ${count} data points`);
                sendJson(res, 200, {});
                return;
            }
...
async function handleHookEvent(url, req, repo, res, verbose, lastSessionEnd) {
    const body = await readBody(req);
    try {
        const payload = JSON.parse(body);
        const eventName = routeToEventName(url);
        if (verbose)
            log(`hook: ${eventName} session=${payload.session_id ?? "?"}`);
        ...
        if (eventName === "SessionEnd" && payload.session_id && lastSessionEnd) {
            ...
            if (now - last < SESSION_END_DEDUP_MS) {
                if (verbose)
                    log(`  -> suppressed duplicate SessionEnd for ${payload.session_id}`);
        ...
        if (eventName === "SessionEnd" && payload.transcript_path) {
            try {
                ...
                if (verbose)
                    log(`  -> ingested transcript: ${payload.transcript_path}${projectPath ? ` (project: ${projectPath})` : ""}`);
            }
            catch (err) {
                if (verbose)
                    log(`  -> transcript ingest failed: ${err}`);
            }
        }
        ...
        if (eventName === "PostToolUse" && payload.tool_name === "Bash") {
            ...
            if (/\bgit\s+(commit|push)\b/.test(cmd)) {
                ...
                if (ctx?.active && ctx.active.length > 0) {
                    clearActiveContext();
                    if (verbose)
                        log(`  -> cleared task context after: ${cmd.slice(0, 60)}`);
                }
            }
        }
...
// ── API handler ──
function handleApiRoute(url, repo, res) {
    ...
    if (path === "/api/tasks/stats") {
        ...
        const mode = qs.get("mode") === "any" ? "any" : "all";
    ...
    if (path === "/api/tasks/turns") {
        ...
        const mode = qs.get("mode") === "all" ? "all" : "any";
        const from = qs.get("from") ?? undefined;
        const to = qs.get("to") ?? undefined;
 ...
 */
function decodeProjectPathFromTranscript(transcriptPath) {
